...
pub async fn handle_clip_result(
    ...
    if clip_score < clip_threshold {
        let session =
            fetch_and_reject(pool, image_uuid, "clip_threshold", &clip_json, "clip").await?;
    ...
    let updated =
        merge_scorer_result(pool, image_uuid, "clip", &clip_json, score_timeout_secs).await?;
...
pub async fn handle_artifact_result(
    ...
    let updated = merge_scorer_result(
        pool,
        image_uuid,
        "artifact",
        &artifact_json,
...
pub async fn handle_vlm_result(
    ...
    let updated =
        merge_scorer_result(pool, image_uuid, "vlm", &vlm_json, score_timeout_secs).await?;
    if let Some(session) = updated {
        try_aggregate(pool, publisher, image_uuid, &session).await?;
    }
    Ok(())
...
async fn merge_scorer_result(
    ...
    let clip = if column == "clip" {
        Some(json_value)
    } else {
        row.clip.as_deref()
    };
    let artifact = if column == "artifact" {
        Some(json_value)
    } else {
        row.artifact.as_deref()
    };
    let vlm = if column == "vlm" {
        Some(json_value)
    } else {
        row.vlm.as_deref()
    };
...
async fn fetch_and_reject(
    ...
    let clip = if scorer_column == "clip" {
        Some(scorer_json)
    } else {
        row.clip.as_deref()
    };
    let artifact = if scorer_column == "artifact" {
        Some(scorer_json)
    } else {
        row.artifact.as_deref()
    };
    let vlm = if scorer_column == "vlm" {
        Some(scorer_json)
    } else {
        row.vlm.as_deref()
    };
...
async fn publish_cancel(
    ...
    publisher
        .publish(&format!("scorer.events/cancel.{}", image_uuid), &payload)
        .await
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        clip_threshold,
        artifact_threshold, score_timeout, "Aggregator starting"
    );
    ...
    let mut clip_receiver =
        Receiver::attach(&mut session, "aggregator-clip", "aggregator.clip.queue").await?;
    ...
    .await?;
    let mut vlm_receiver =
        Receiver::attach(&mut session, "aggregator-vlm", "aggregator.vlm.queue").await?;
